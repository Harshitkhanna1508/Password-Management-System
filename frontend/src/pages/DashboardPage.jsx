  ...
  return (
    <div className="space-y-10 pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <h1 className="text-4xl font-bold tracking-tight mb-2 text-[var(--color-text)]">Vault Dashboard</h1>
          <p className="text-[var(--color-text-muted)] font-medium">Welcome back. Your digital life is under maximum protection.</p>
        </div>
        <button 
          onClick={() => handleOpenModal()}
          className="bg-[#6366f1] text-white px-8 py-4 rounded-full text-[11px] font-black uppercase tracking-[0.2em] shadow-xl shadow-indigo-500/20 hover:scale-105 active:scale-95 transition-all flex items-center gap-3"
        >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          ...
          <div key={i} className="bg-[var(--color-surface)]/40 backdrop-blur-xl border border-[var(--color-border)] rounded-[32px] p-8 shadow-sm dark:shadow-2xl relative overflow-hidden group hover:bg-[var(--color-surface)]/60 transition-all">
            ...
            <div className="flex items-center gap-4">
              <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-white shadow-lg ${stat.color} bg-opacity-20`}>
                <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={stat.icon} />
                </svg>
              </div>
              <div>
                <p className="text-[10px] font-black text-[var(--color-text-muted)] uppercase tracking-widest">{stat.label}</p>
                <p className="text-3xl font-black text-[var(--color-text)]">{stat.count}</p>
              </div>
            </div>
          </div>
        ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 space-y-6">
          <div className="bg-[var(--color-surface)]/40 backdrop-blur-xl border border-[var(--color-border)] rounded-[40px] p-10 shadow-sm dark:shadow-2xl overflow-hidden min-h-[500px]">
            <div className="flex items-center justify-between mb-8">
              <h3 className="text-xl font-bold tracking-tight">Encrypted Items</h3>
              <div className="flex items-center gap-4">
                <input 
                  type="text" 
                  ...
                  className="bg-[var(--color-bg)] border border-[var(--color-border)] rounded-xl px-4 py-2 text-xs focus:outline-none focus:border-[#6366f1]/50 text-[var(--color-text)]"
                />
              </div>
            </div>
